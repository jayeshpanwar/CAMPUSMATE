...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            // This function simulates fetching data you'd get after a user logs in.
        ...
        socket.onmessage = (event) => {
            ...
            switch (action) {
                ...
                case 'GRADES_UPDATED':
                    ...
                    break;
                case 'NEW_MESSAGE':
                    // data = { conversationId: 1, message: { sender: '...', text: '...' } }
                    setAllMessages(prev => ({
                        ...prev,
    ...
    const handleGenerateStudyPlan = async () => {
        ...
        try {
            if (apiKey === "YOUR_GOOGLE_AI_API_KEY") {
                throw new Error("Please replace 'YOUR_GOOGLE_AI_API_KEY' with your actual API key in the code.");
            }
            ...
            if (generatedText) {
                ...
                setStudyPlan('Could not generate a study plan at this time. The model returned an empty response.');
            }
        } catch (error) {
    ...
    const handleGenerateDaySummary = async (events, date) => {
        ...
        const prompt = `Act as a personal assistant. Summarize the following schedule for ${date.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })} in a single, friendly, and easy-to-read paragraph. Mention the times and importance of the events. \n\nEvents:\n${eventsString}`;
        const payload = { contents: [{ parts: [{ text: prompt }] }] };
        ...
        try {
            ...
            const generatedText = result.candidates?.[0]?.content?.parts?.[0]?.text;
             if (generatedText) {
                ...
                setDaySummary('Could not generate a summary at this time. The model returned an empty response.');
            }
        } catch (error) {
            console.error('Gemini API error:', error);
            setDaySummary(`An error occurred while generating the summary.\n\nDetails: ${error.message}`);
        } finally {
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-sans flex">
            ...
            {isStudyPlanModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-2xl w-full animate-fade-in-up">
